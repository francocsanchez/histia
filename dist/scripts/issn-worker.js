...
const { verifyIssnAffiliation } = runtimeRequire("../src/lib/issn");
const { IssnAutomationControlModel } = runtimeRequire("../src/models/issn-automation-control");
const { acquireIssnWorkerLease, createIssnWorkerInstanceId, deactivateIssnPatient, finishIssnRun, getIssnPatients, getIssnAutomationStatus, releaseIssnWorkerLease, recoverInterruptedIssnRun, setIssnRunTotal, startIssnRun, updateIssnRunProgress, } = runtimeRequire("../src/services/issn-automation");
...
async function shouldRunScheduled() {
    ...
    if (status.status === "running" || clock.hour !== 2 || clock.minute !== 0)
        return false;
...
async function run(origin) {
    if (!(await startIssnRun(origin)))
        return;
    if (origin === "scheduled")
        await markScheduledRunDate();
    try {
        ...
        for (const patient of patients) {
            ...
            if (patient !== patients[patients.length - 1])
    ...
    while (active) {
        try {
            if (await acquireIssnWorkerLease(workerId)) {
                ...
                const status = await getIssnAutomationStatus();
                if (status.manualRunRequestedAt && status.status !== "running")
                    await run("manual");
                else if (await shouldRunScheduled())
                    await run("scheduled");
            }
        }
        catch (error) {
