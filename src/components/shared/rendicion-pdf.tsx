    ...
    <Document title={filename} author="Histia" subject="Detalle de liquidación profesional">
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.kpis}>
          <Kpi label="Pacientes" value={String(rendition.pacientesUnicos)} /><Kpi label="Atenciones" value={String(rendition.atencionesUnicas)} /><Kpi label="Códigos OK" value={currency(payment.totalPagoCodigosCentavos)} /><Kpi label="Coseguros" value={currency(payment.totalCoseguroOdontoCentavos)} /><Kpi label="Ortodoncia" value={currency(payment.totalOrtodonciaCentavos)} /><Kpi label="Créditos" value={`+ ${currency(payment.totalCreditosCentavos)}`} /><Kpi label="Débitos" value={`- ${currency(payment.totalDebitosCentavos)}`} /><Kpi label="Conceptos" value={String(payment.quantityConceptsPaid)} />
        </View>
        ...
        <View style={styles.reconciliation} wrap={false}><ReconRow label="Total códigos" value={currency(payment.totalPagoCodigosCentavos)} /><ReconRow label="Total coseguros" value={currency(payment.totalCoseguroOdontoCentavos)} /><ReconRow label="Total ortodoncia" value={currency(payment.totalOrtodonciaCentavos)} /><ReconRow label="Créditos" value={`+ ${currency(payment.totalCreditosCentavos)}`} /><ReconRow label="Débitos" value={`- ${currency(payment.totalDebitosCentavos)}`} /><View style={[styles.reconRow, styles.reconTotal]}><Text>Neto pagado</Text><Text>{currency(payment.totalNetoPagarCentavos)}</Text></View></View>
